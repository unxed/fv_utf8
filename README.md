...
An attempt to adopt Free Vision TUI library from [Free Pascal](https://www.freepascal.org/) from using 1-byte character encodings to UTF-8 on Linux. Current state: test app builds, runs and is able to show its interface properly. All controls of demo app are working (text editor needs some improvements though, but is already able to display utf-8 encoded text opened from file).
